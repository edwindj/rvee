...
{{#fns}}
[manualfree]
fn {{pkg}}_{{name}}({{#input}}{{^first}}, {{/first}}{{name}} C.SEXP{{/input}}) C.SEXP {
  ...
  {{/input}}
  {{#result}}
  res := {{name}}({{#input}}{{^first}}, {{/first}}{{#mut}}mut {{/mut}}i_{{name}}_v{{/input}})

  //wrap output
  return r.from_{{result}}(res)
  {{/result}}
  {{^result}}
  {{name}}({{#input}}{{^first}}, {{/first}}{{#mut}}mut {{/mut}}i_{{name}}_v{{/input}})
  return r.null_value
  {{/result }}
}
